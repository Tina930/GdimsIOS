...
class ViewController: UIViewController, UITextFieldDelegate {
    ...
    @IBOutlet weak var ipNum: UITextField!
    ...
    override func viewDidLoad() {
        ...
        if ((userDefault.getUser(forKey: "isSave") as String!) != nil) {
            self.phone.text = userDefault.getUser(forKey: "phoneNum")
            self.ipNum.text = userDefault.getUser(forKey: "ips")
            self.portNum.text = userDefault.getUser(forKey: "ports")
        }
//        macroRequst()
    }
    ...
    @IBAction func loginBtn(_ sender: Any) {
        print("123")
        /*获取输入框文本*/
        phoneNum = phone.text!.trimmingCharacters(in: .whitespaces)
        ips = ipNum.text!.trimmingCharacters(in: .whitespaces)
        ports = portNum.text!.trimmingCharacters(in: .whitespaces)
        /*设置存储信息*/
